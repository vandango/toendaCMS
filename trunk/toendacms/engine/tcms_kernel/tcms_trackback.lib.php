...
/**
 * toendaCMS Trackback Class
 *
 * This class is used to provide a trackback
 * class.
 ...
 * recieve                    -> Produces XML response for trackbackers with ok/error message.
 * fetch                      -> Feteched trackback information and produces an RSS-0.91 feed.
 * rdf_autodiscover           -> Produces embedded RDF representing metadata about the entry,
 *                               allowing clients to auto-discover the TrackBack Ping URL.
 * auto_discovery             -> Search text for links, and searches links for trackback URLs.
 * RFC822_from_datetime       -> Converts MySQL datetime to a standart RFC 822 date format
 * xml_safe                   -> Converts a string into an XML-safe string (replaces &, <, >, " and ')
 * cut_short                  -> Cuts a string short (with "...") accroding to $max_length...
 *
 */
...
class tcms_trackback{
	...
	function __construct($blog_name, $author, $encoding = 'UTF-8'){
		...
		if(isset($_GET['id']))      { $this->m_get_id  = $HTTP_GET_VARS['id']; }
		if(isset($_POST['id']))     { $this->m_post_id = $HTTP_POST_VARS['id']; }
		if(isset($_POST['url']))    { $this->m_url     = $HTTP_POST_VARS['url']; }
		if(isset($_POST['title']))  { $this->m_title   = $HTTP_POST_VARS['url']; }
		if(isset($_POST['excerpt'])){ $this->m_excerpt = $HTTP_POST_VARS['excerpt']; }
	}
	...
	/**
	 * Sends a ping to a specified url and allows
	 * clients to auto-discover the ping url.
	 * 
	 * @param String $tb
	 * @param String $url
	 * @param String $title = ''
	...
	function ping($tb, $url, $title = '', $excerpt = ''){
		...
		// Set default values
		if(empty($title)){ $title = 'Trackbacking your entry...'; }
		if(empty($excerpt)){ $excerpt = 'I found your entry interesting do I´ve added a Trackback to it on my weblog :)'; }
		...
		if((isset($target['port']) && !is_numeric($target['port'])) || (!isset($target['port']))){ $target['port'] = 80; }
		...
		$tb_send = 'url='.rawurlencode($url).'&amp;title='.rawurlencode($title).'&amp;blog_name='.rawurlencode($this->m_blog_name).'&amp;excerpt='.rawurlencode($excerpt);
		// Send the trackback
		...
		// Gather result
		while(!feof($tb_sock)){ $response .= fgets($tb_sock, 128); }
	...
	function recieve($success = false, $err_response = ''){
		...
		$return = '<?xml version="1.0" encoding="'.$this->m_encoding.'"?>'."\n";
		$return .= "<response> \n";
